...
@Autonomous(name="Pos1MediumPoleWithLogic", group="Exercises")
//@Disabled
public class Pos1MediumPoleWithLogic extends LinearOpMode
{
	...
	public void runOpMode() throws InterruptedException
	{
		...
		Thread t1 = new Thread(){public void run(){swayleft(58f); idle();}};
		Thread t2 = new Thread(){public void run(){armMove(2); idle();}};
		t1.start();
		t2.start();
		try{t1.join();} catch (InterruptedException e){e.printStackTrace();}
		try{t2.join();} catch (InterruptedException e){e.printStackTrace();}
		
		Thread t3 = new Thread(){public void run(){moveforeward(12.5f); idle();}};
		Thread t4 = new Thread(){public void run(){armServo.setPosition(0.535); idle();}};
		t3.start();
		t4.start();		
		...
		if(tagOfInterest != null){
				if(tagOfInterest.id == LEFT){sideConeShort2();}
				else if(tagOfInterest.id == MIDDLE){sideCone();}
				else if(tagOfInterest.id == RIGHT){sideCone();}
			}
		else {sideConeShort2();} 

		Thread p1 = new Thread(){public void run(){armServo.setPosition(0.976);}};
		Thread p2 = new Thread(){public void run(){
			if(tagOfInterest != null){
				if(tagOfInterest.id == LEFT){movebackward(12);}
				else if(tagOfInterest.id == MIDDLE){movebackward(12);}
				else if(tagOfInterest.id == RIGHT){moveforeward(12);}
			}
			else {moveforeward(12);} 
		}};
		p1.start();
		p2.start();		
		...
		/*Thread t1 = new Thread(){public void run(){moveforeward(55f);}};
		Thread t2 = new Thread(){public void run(){armMove(3);}};
		t1.start();
		t2.start();
	...
	private void sideCone(){
		...
		Thread t8 = new Thread(){public void run(){moveforeward(36f); idle();}};
	 ...
	 private void sideConeShort1() {
		  Thread t5 = new Thread(){public void run(){movebackwardColor(38.5f); idle();}};
		Thread t6 = new Thread(){public void run(){armMove(1); idle();}};
		Thread t7 = new Thread(){public void run(){armServo.setPosition(0.976); idle();}};
		t5.start();
		t6.start();	
		...
		Thread t8 = new Thread(){public void run(){moveforeward(12.5f); idle();}};
		Thread t0 = new Thread(){public void run(){armServo.setPosition(0.535); idle();}};
	 ...
	 private void sideConeShort2() {
		...
		Thread t8 = new Thread(){public void run(){moveforeward(13f); idle();}};
		Thread t0 = new Thread(){public void run(){armServo.setPosition(0.535); idle();}};
	...
	private void moveforewardColor(float distance){
		int value = getTargetPosition(distance);
		setEncoders(value, value, value, value);
		//while ((leftMotor.isBusy() && colorSensor2.alpha()<220) && !isStopRequested()){
		while ((leftMotor.isBusy()) && !isStopRequested()){
			...
			telemetry.update();
			if (!(leftMotor.isBusy() && colorSensor2.getDistance(DistanceUnit.MM)>28 && !isStopRequested())){
				break;
			}
