...
@Autonomous(name="Pos1Range_PBlue", group="Exercises")
//@Disabled
public class Pos1Range_PBlue extends LinearOpMode
{
	...
	public void runOpMode() throws InterruptedException
	{
		...
		Thread t1 = new Thread(){public void run(){swayleft(57f); idle();}};
	...
	private void armMove(int targetPos){
		if (targetPos == 0) {
			...
			while ((!bodyTouch.isPressed())&&(!armTouch.isPressed())&& !isStopRequested()){
				setPowerArm(-0.6);
			if(!((!bodyTouch.isPressed())&&(!armTouch.isPressed())&& !isStopRequested())&&timer.seconds()>1.3){
					break;
				}
			}
			armPos = 0;
