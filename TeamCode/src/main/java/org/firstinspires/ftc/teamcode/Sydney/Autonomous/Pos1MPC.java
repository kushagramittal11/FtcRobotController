...
@Autonomous(name="Pos1MPC", group="Exercises")
//@Disabled
public class Pos1MPC extends LinearOpMode
{
	...
	private double pidweight, colorThresh = 6;
	...
	public void runOpMode() throws InterruptedException
	{
		...
		//PARKING LOGIC
		/*Thread p1 = new Thread(){public void run(){armServo.setPosition(0.06);}};
		Thread p2 = new Thread(){public void run(){
			if(tagOfInterest != null){
				...
				else if(tagOfInterest.id == RIGHT){moveforeward(36);}
			}
			else {movebackward(36);} 
		}};
		p1.start();
		p2.start();		
	...
	private void moveforewardColor(float distance){
		...
		while (leftMotor.isBusy() && !isStopRequested()){
			...
			telemetry.addData("avgEncoder<value-200", colorSensor2.alpha());
			telemetry.update();
			//if (!((leftMotor.isBusy() && !isStopRequested()) || (colorSensor2.alpha()<120 && !isStopRequested()))){
			if (!(leftMotor.isBusy() && colorSensor2.alpha()>colorThresh && !isStopRequested())){
				break;
			}
