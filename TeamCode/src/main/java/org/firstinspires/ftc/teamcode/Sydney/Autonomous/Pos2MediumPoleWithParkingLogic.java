...
@Autonomous(name="Pos2MediumPoleWithParkingLogic", group="Exercises")
//@Disabled
public class Pos2MediumPoleWithParkingLogic extends LinearOpMode
{
	...
	public void runOpMode() throws InterruptedException
	{
		...
		colorSensor2 = hardwareMap.get(ColorSensor.class, "colorSensor3");
		...
		Thread t1 = new Thread(){public void run(){swayleft(55.5f); idle();}};
		...
		Thread t3 = new Thread(){public void run(){movebackward(12.5f); idle();}};
		...
		if(tagOfInterest != null){
				if(tagOfInterest.id == LEFT){sideCone();}
				else if(tagOfInterest.id == MIDDLE){sideCone();}
				else if(tagOfInterest.id == RIGHT){sideConeShort2();}
			}
		else {sideConeShort2();} 
		...
		Thread p2 = new Thread(){public void run(){
			if(tagOfInterest != null){
				...
				else if(tagOfInterest.id == MIDDLE){moveforeward(12);}
	...
	private void sideCone(){
		Thread t5 = new Thread(){public void run(){moveforewardColor(15f); idle();}};
		Thread t6 = new Thread(){public void run(){armMove(1); idle();}};
		Thread t7 = new Thread(){public void run(){armServo.setPosition(0.06); idle();}};
		t5.start();
		t6.start();	
		t7.start();
	 ...
	 private void sideConeShort1() {
		  Thread t5 = new Thread(){public void run(){moveforewardColor(38.5f); idle();}};
		Thread t6 = new Thread(){public void run(){armMove(1); idle();}};
		Thread t7 = new Thread(){public void run(){armServo.setPosition(0.06); idle();}};
		t5.start();
		t6.start();	
		...
		Thread t8 = new Thread(){public void run(){movebackward(12.5f); idle();}};
		Thread t0 = new Thread(){public void run(){armServo.setPosition(0.535); idle();}};
	...
	private void moveforewardColor(float distance){
		int value = getTargetPosition(distance);
		setEncoders(value, value, value, value);
		while ((leftMotor.isBusy() && colorSensor2.alpha()<185) && !isStopRequested()){
			idle();
			int avgEncoder = (leftMotor.getCurrentPosition() + rightMotor.getCurrentPosition() + backleftMotor.getCurrentPosition() + backrightMotor.getCurrentPosition())/4;
			...
			telemetry.update();
			if (!((leftMotor.isBusy() && !isStopRequested()) || (colorSensor2.alpha()<120 && !isStopRequested()))){
				break;
			}
