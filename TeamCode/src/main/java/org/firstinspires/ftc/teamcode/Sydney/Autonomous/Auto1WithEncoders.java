...
@Autonomous(name="Auto1WithEncoders", group="Exercises")
//@Disabled
public class Auto1WithEncoders extends LinearOpMode
{
	...
	public void runOpMode() throws InterruptedException
	{
		...
		colorSensor2 = hardwareMap.get(DistanceSensor.class, "colorSensor3");
		...
		//AUTONOMOUS PATH
		moveforewardColor(24);
		/*Thread t1 = new Thread(){public void run(){swayright(58f); idle();}};
		Thread t2 = new Thread(){public void run(){armMove(3); idle();}};
		t1.start();
		t2.start();		
		...
		//PARKING LOGIC
	/*	Thread p1 = new Thread(){public void run(){armMove(1); armServo.setPosition(0.976);}};
		Thread p2 = new Thread(){public void run(){
			if(tagOfInterest != null){
				if(tagOfInterest.id == LEFT){moveforeward(36);}
				else if(tagOfInterest.id == MIDDLE){moveforeward(12);}
				else if(tagOfInterest.id == RIGHT){movebackward(12);}
			}
			else {moveforeward(36);} 
		}};
		p1.start();
		p2.start();		
	...
	private void sideCone(){
		...
		Thread t9 = new Thread(){public void run(){armMove(3); idle();}};
	...
	private void moveforewardColor(float distance){
		int value = getTargetPosition(distance);
		setEncoders(value, value, value, value);
		while ((leftMotor.isBusy() && colorSensor2.getDistance(DistanceUnit.MM)<25) && !isStopRequested()){
			idle();
			int avgEncoder = (leftMotor.getCurrentPosition() + rightMotor.getCurrentPosition() + backleftMotor.getCurrentPosition() + backrightMotor.getCurrentPosition())/4;
			...
			telemetry.update();
			if (!((leftMotor.isBusy() && !isStopRequested()) || (colorSensor2.getDistance(DistanceUnit.MM)<25 && !isStopRequested()))){
				break;
			}
