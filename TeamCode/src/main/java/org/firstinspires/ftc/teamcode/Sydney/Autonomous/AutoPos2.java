...
@Autonomous(name="AutoPos2", group="Exercises")
//@Disabled
public class AutoPos2 extends LinearOpMode
{
	...
	double globalAngle, power = .63, correctionL, correctionR, rotation, correctionDistance, correctionHeading, correction;
	...
	public void runOpMode() throws InterruptedException
	{
		...
		Thread t1 = new Thread(){public void run(){swayright(59.5f); idle();}};
		...
		Thread t3 = new Thread(){public void run(){moveforeward(11.5f); idle();}};
		...
		//PARKING LOGIC
		Thread p1 = new Thread(){public void run(){armMove(1); armServo.setPosition(0.06);}};
		Thread p2 = new Thread(){public void run(){
			if(tagOfInterest != null){
				if(tagOfInterest.id == LEFT){moveforeward(12);}
				else if(tagOfInterest.id == MIDDLE){movebackward(12);}
				else if(tagOfInterest.id == RIGHT){movebackward(36);}
			}
	...
	private void moveforewardColor(float distance){
		int value = getTargetPosition(distance);
		setEncoders(value, value, value, value);
		while ((leftMotor.isBusy() && colorSensor2.alpha()<225) && !isStopRequested()){
			idle();
			int avgEncoder = (leftMotor.getCurrentPosition() + rightMotor.getCurrentPosition() + backleftMotor.getCurrentPosition() + backrightMotor.getCurrentPosition())/4;
